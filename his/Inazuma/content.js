...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mdFileName = (() => {
        ...
        const region = segments[segments.length - 2] || 'Inazuma';
    ...
    function normalizeInline(raw) {
        ...
        s = s
            ...
            .replaceAll(/\[\[\[MDIMG:([^\]]+)\]\]\]/g, (match, data) => {
                ...
                return `<img src="${fixedSrc}" alt="${alt}" class="timeline-image-inline" style="display:block;margin:0 auto 8px;max-width:100%;height:auto;"><span class="image-caption">${injectFootnoteTooltips(normalizeInline(alt), {})}</span>`;
            })
        ...
        while (i < allLines.length) {
            ...
            if (/^!\[[^\]]*\]\([^)]*\)/.test(trimmed) && !/^!\[(Imagebg|Introbg)/i.test(trimmed)) {
                ...
                if (m) {
                    ...
                    wrapper.innerHTML = `<img src="${fixedSrc}" alt="${escapeHtml(alt)}" style="max-width:100%;height:auto;border-radius:4px;"><p class="image-caption" style="color:#8B7355;font-size:0.9em;margin-top:8px;">${injectFootnoteTooltips(normalizeInline(alt), {})}</p>`;
                ...
                while (i < contentLines.length) {
                    ...
                    if (/^!\[[^\]]*\]\([^)]*\)/.test(trimmed) && !/^!\[(Imagebg|Introbg)/i.test(trimmed)) {
                        ...
                        if (m) {
                            ...
                            wrapper.innerHTML = `<img src="${fixedSrc}" alt="${escapeHtml(alt)}" style="max-width:calc(100% - 100px);display:block;margin:0 auto;"><p class="image-caption">${injectFootnoteTooltips(normalizeInline(alt), {})}</p>`;
                            appendToCurrent(wrapper);
                            i++;
                            continue;
                        }
